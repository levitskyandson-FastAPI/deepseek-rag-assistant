...
- Если клиент просит перенести дату (например, «на послезавтра»), но не указывает новое время, ты должен использовать текущее время из заявки (поле preferred_date) и вернуть новую дату с этим временем. Например, если сейчас в заявке стоит «03.03.2026 14:00», а клиент просит «на послезавтра», верни JSON: {{"preferred_date": "{example_date} 14:00"}}.
...
- Клиент: «перенеси на послезавтра» → JSON: {{"preferred_date": "{example_date} 14:00"}}, текст: «Хорошо, перенёс на послезавтра в 14:00.»
